...
function row(label: string, value: string): string {
  return `<tr>
    <td style="padding:9px 18px 9px 0;border-bottom:1px solid ${HAIRLINE};color:${MUTED_COLOR};font-family:${SANS};font-size:12px;font-weight:600;vertical-align:top;white-space:nowrap;text-transform:uppercase;letter-spacing:0.09em;line-height:21px">${label}</td>
    <td style="padding:9px 0;border-bottom:1px solid ${HAIRLINE};color:${HEADING_COLOR};font-family:${SANS};font-size:15px;line-height:21px">${value}</td>
  </tr>`;
}
...
export function newLeadEmail(data: LeadEmailData): { subject: string; html: string } {
  ...
  const body = `
    <p style="margin:0 0 20px;color:${TEXT_COLOR};font-size:16px;line-height:1.6">A new lead just came in from the website.</p>
    ${dataTable(
      row('Name', data.name) +
...
export function newSupportTicketEmail(data: SupportTicketEmailData): { subject: string; html: string } {
  const body = `
    <p style="margin:0 0 20px;color:${TEXT_COLOR};font-size:16px;line-height:1.6">A client has submitted a new support ticket.</p>
    ${dataTable(
...
export function ticketStatusUpdateEmail(data: TicketStatusUpdateData): { subject: string; html: string } {
  ...
  const body = `
    <p style="margin:0 0 14px;color:${HEADING_COLOR};font-size:16px;font-weight:600;line-height:1.6">Hi ${data.clientName},</p>
    <p style="margin:0 0 20px;color:${TEXT_COLOR};font-size:16px;line-height:1.6">Your support ticket has been updated.</p>
    ${dataTable(
...
export function talentApplicationReceivedEmail(data: TalentApplicationData): { subject: string; html: string } {
  const body = `
    <p style="margin:0 0 14px;color:${HEADING_COLOR};font-size:16px;font-weight:600;line-height:1.6">Hi ${data.fullName},</p>
    <p style="margin:0 0 16px;color:${TEXT_COLOR};font-size:16px;line-height:1.6">Thank you for applying to <strong style="color:${BRAND_MAGENTA}">CreativeMinds</strong> &mdash; FreakingMinds' curated network of creative professionals.</p>
    <p style="margin:0 0 16px;color:${TEXT_COLOR};font-size:16px;line-height:1.6">We've received your application and our team will review it within 2&ndash;3 business days. You'll receive an email once a decision has been made.</p>
    <div style="margin:20px 0;padding:15px 19px;border:1px solid ${HAIRLINE};border-radius:10px;border-left:3px solid ${BRAND_MAGENTA}">
      <p style="margin:0;color:${TEXT_COLOR};font-size:14px;line-height:1.5"><strong>What happens next?</strong><br>Our team reviews every application personally. If approved, you'll get a public profile on our talent network and access to project opportunities.</p>
    </div>
    <p style="margin:0;color:${TEXT_COLOR};font-size:16px;line-height:1.6">Best,<br><strong>The FreakingMinds Team</strong></p>
  `;
...
export function talentApplicationTeamEmail(data: TalentApplicationData): { subject: string; html: string } {
  const body = `
    <p style="margin:0 0 20px;color:${TEXT_COLOR};font-size:16px;line-height:1.6">A new CreativeMinds application needs review.</p>
    ${dataTable(
...
export function talentApprovedEmail(data: TalentApprovedData): { subject: string; html: string } {
  ...
  const credentialsSection = data.tempPassword && data.portalEmail ? `
    <div style="margin:20px 0;padding:20px 24px;background:#f0fdf4;border-radius:10px;border:1px solid #bbf7d0">
      <p style="margin:0 0 12px;color:${HEADING_COLOR};font-size:14px;font-weight:600;letter-spacing:0.01em">Your Portal Login Credentials</p>
      <table cellpadding="0" cellspacing="0" style="width:100%">
        <tr>
          <td style="padding:4px 0;color:${MUTED_COLOR};font-size:14px;width:80px">Email</td>
          <td style="padding:4px 0;color:${HEADING_COLOR};font-size:15px;font-weight:600">${data.portalEmail}</td>
        </tr>
        <tr>
          <td style="padding:4px 0;color:${MUTED_COLOR};font-size:14px">Password</td>
          <td style="padding:4px 0"><code style="background:#e2e8f0;padding:2px 8px;border-radius:4px;font-size:15px;font-weight:600;color:${HEADING_COLOR};letter-spacing:0.5px">${data.tempPassword}</code></td>
        </tr>
      </table>
      <p style="margin:10px 0 0;color:${MUTED_COLOR};font-size:12px">Please change your password after first login.</p>
    </div>
    ${ctaButton('Login to Portal', portalUrl, '#22c55e')}
  ` : '';

  const body = `
    <p style="margin:0 0 14px;color:${HEADING_COLOR};font-size:16px;font-weight:600;line-height:1.6">Congratulations, ${data.fullName}!</p>
    <p style="margin:0 0 16px;color:${TEXT_COLOR};font-size:16px;line-height:1.6">Your <strong style="color:${BRAND_MAGENTA}">CreativeMinds</strong> application has been approved. Your public talent profile is now live on our network.</p>
    <div style="margin:20px 0;padding:15px 19px;border:1px solid ${HAIRLINE};border-radius:10px;border-left:3px solid #22c55e">
      <p style="margin:0;color:${TEXT_COLOR};font-size:14px;line-height:1.5"><strong>What this means:</strong><br>Clients and brands browsing our talent network can now discover your profile. You may be contacted for project opportunities that match your skills.</p>
    </div>
    ${credentialsSection}
    ${ctaButton('View Your Profile', profileUrl, data.tempPassword ? BRAND_MAGENTA : '#22c55e')}
    <p style="margin:16px 0 0;color:${TEXT_COLOR};font-size:16px;line-height:1.6">Welcome to the network!<br><strong>The FreakingMinds Team</strong></p>
  `;
...
export function talentRejectedEmail(data: TalentRejectedData): { subject: string; html: string } {
  const body = `
    <p style="margin:0 0 14px;color:${HEADING_COLOR};font-size:16px;font-weight:600;line-height:1.6">Hi ${data.fullName},</p>
    <p style="margin:0 0 16px;color:${TEXT_COLOR};font-size:16px;line-height:1.6">Thank you for your interest in joining <strong style="color:${BRAND_MAGENTA}">CreativeMinds</strong>. After carefully reviewing your application, we've decided not to move forward at this time.</p>
    <p style="margin:0 0 16px;color:${TEXT_COLOR};font-size:16px;line-height:1.6">This doesn't reflect on your talent &mdash; we may have specific needs that didn't align with your profile right now. You're welcome to reapply in the future as our requirements evolve.</p>
    <p style="margin:0 0 16px;color:${TEXT_COLOR};font-size:16px;line-height:1.6">If you have questions, feel free to reach out at <a href="mailto:hello@freakingminds.in" style="color:${BRAND_MAGENTA};text-decoration:none">hello@freakingminds.in</a>.</p>
    <p style="margin:0;color:${TEXT_COLOR};font-size:16px;line-height:1.6">Wishing you the best,<br><strong>The FreakingMinds Team</strong></p>
  `;
...
export function invoiceCreatedEmail(data: InvoiceEmailData): { subject: string; html: string } {
  ...
  const body = `
    <p style="margin:0 0 20px;color:${TEXT_COLOR};font-size:16px;line-height:1.6">A new invoice has been created.</p>
    ${dataTable(
      row('Invoice #', `<strong>${data.invoiceNumber}</strong>`) +
      row('Client', data.clientName) +
      row('Total', `<strong style="color:${HEADING_COLOR};font-size:17px">${formatted}</strong>`) +
...
export function proposalCreatedEmail(data: ProposalEmailData): { subject: string; html: string } {
  const body = `
    <p style="margin:0 0 20px;color:${TEXT_COLOR};font-size:16px;line-height:1.6">A new proposal has been created.</p>
    ${dataTable(
...
export function contractCreatedEmail(data: ContractEmailData): { subject: string; html: string } {
  ...
  const body = `
    <p style="margin:0 0 20px;color:${TEXT_COLOR};font-size:16px;line-height:1.6">A new contract has been created.</p>
    ${dataTable(
      ...
      (formatted ? row('Value', `<strong style="color:${HEADING_COLOR};font-size:17px">${formatted}</strong>`) : '') +
...
export function proposalStatusEmail(data: ProposalStatusEmailData): { subject: string; html: string } {
  ...
  const body = `
    <p style="margin:0 0 20px;color:${TEXT_COLOR};font-size:16px;line-height:1.6">A client has responded to a proposal.</p>
    ${dataTable(
...
export function proposalSentToClientEmail(data: ProposalSentToClientData): { subject: string; html: string } {
  const body = `
    <p style="margin:0 0 14px;color:${HEADING_COLOR};font-size:16px;font-weight:600;line-height:1.6">Hi ${escHtml(data.clientName)},</p>
    <p style="margin:0 0 20px;color:${TEXT_COLOR};font-size:16px;line-height:1.6">A new proposal is ready for your review.</p>
    ${dataTable(
      row('Proposal', `<strong>${escHtml(data.title)}</strong>`) +
      (data.proposalNumber ? row('Number', escHtml(data.proposalNumber)) : '') +
      row('Status', badge('READY FOR REVIEW', '#3b82f6'))
    )}
    <p style="margin:16px 0 0;color:${TEXT_COLOR};font-size:15px;line-height:1.5">Please review the proposal in your client portal and let us know your thoughts.</p>
...
export function invoiceStatusEmail(data: InvoiceStatusEmailData): { subject: string; html: string } {
  ...
  const body = `
    <p style="margin:0 0 14px;color:${HEADING_COLOR};font-size:16px;font-weight:600;line-height:1.6">Hi ${escHtml(data.clientName)},</p>
    <p style="margin:0 0 20px;color:${TEXT_COLOR};font-size:16px;line-height:1.6">${messages[data.status]}</p>
    ${dataTable(
      row('Invoice #', `<strong>${escHtml(data.invoiceNumber)}</strong>`) +
      row('Amount', `<strong style="color:${HEADING_COLOR};font-size:17px">${formatted}</strong>`) +
      (data.dueDate ? row('Due Date', data.dueDate) : '') +
...
export function documentSharedEmail(data: DocumentSharedEmailData): { subject: string; html: string } {
  const body = `
    <p style="margin:0 0 14px;color:${HEADING_COLOR};font-size:16px;font-weight:600;line-height:1.6">Hi ${escHtml(data.clientName)},</p>
    <p style="margin:0 0 20px;color:${TEXT_COLOR};font-size:16px;line-height:1.6">A new document has been shared with you.</p>
    ${dataTable(
      row('File', `<strong>${escHtml(data.fileName)}</strong>`)
    )}
    <p style="margin:16px 0 0;color:${TEXT_COLOR};font-size:15px;line-height:1.5">You can access this document in your client portal.</p>
...
export function contractStatusEmail(data: ContractStatusEmailData): { subject: string; html: string } {
  ...
  const body = `
    <p style="margin:0 0 20px;color:${TEXT_COLOR};font-size:16px;line-height:1.6">Contract status has been updated.</p>
    ${dataTable(
      ...
      <table width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 20px;border:1px solid ${BORDER_COLOR};border-radius:10px">
        <tr><td style="padding:16px 18px">
          ...
          <table width="100%" cellpadding="0" cellspacing="0">
            <tr>
              <td style="color:${HEADING_COLOR};font-size:16px;font-weight:600">${d.label}</td>
              <td align="right" style="color:${color};font-size:19px;font-weight:600">${d.score}</td>
            </tr>
          </table>
          ...
          <div style="color:${TEXT_COLOR};font-size:14px;line-height:1.6">${d.recommendation}</div>
        </td></tr>
      </table>`;
    })
    .join('');
  ...
  const body = `
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 28px;border:1px solid ${HAIRLINE};border-radius:14px">
      <tr><td align="center" style="padding:26px 20px">
        <div style="color:${MUTED_COLOR};font-size:12px;letter-spacing:1px;text-transform:uppercase;margin:0 0 8px">Your score</div>
        <div style="color:${overallColor};font-family:${SERIF};font-size:54px;font-weight:400;line-height:1">${data.overall}<span style="color:${MUTED_COLOR};font-size:22px;font-weight:500">/100</span></div>
        <div style="color:${overallColor};font-size:16px;font-weight:600;margin:6px 0 0">${data.bandLabel}</div>
      </td></tr>
    </table>
    ...
    <p style="margin:0 0 4px;color:${TEXT_COLOR};font-size:15px;line-height:1.6">
