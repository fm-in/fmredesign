...
export function renderShell(input: ShellInput): string {
  ...
  return [
    ...
    `<table role="presentation" class="s" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="${LIGHT_BG}" style="width:100%;background-color:${LIGHT_BG};">`,
    `<tr><td class="s" align="center" bgcolor="${LIGHT_BG}" style="background-color:${LIGHT_BG};padding:44px 16px;">`,
    ...
    // Body
    `<tr><td class="s c" bgcolor="${CARD_BG}" style="background-color:${CARD_BG};padding:34px 40px 36px;font-family:${FONT_STACK};font-size:16px;line-height:1.6;color:${BODY_TEXT};">`,
    ...
    `<a class="f" href="${escapeHtml(input.cta.url)}" style="display:inline-block;color:#ffffff;text-decoration:none;font-weight:600;font-size:15px;line-height:1;letter-spacing:0.02em;font-family:${FONT_STACK};">${escapeHtml(input.cta.label)}</a>`,
    '</td>',
    '</tr>',
    '</table>',
    '</td></tr>',
    // Footer
    `<tr><td class="s c" bgcolor="${CARD_BG}" style="background-color:${CARD_BG};padding:22px 40px 26px;border-top:1px solid ${HAIRLINE};font-family:${FONT_STACK};font-size:13px;line-height:20px;color:${FOOTER_TEXT};">`,
